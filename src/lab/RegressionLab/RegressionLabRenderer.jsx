...
import { REGRESSION_INTRO_SLIDES } from "../../components/shared/introSlides";
...
const TRAIN_UI_UPDATE_MS = 120;
const TRAIN_FRAME_BUDGET_MS = 7;
const INTRO_LAB_ID = "regression";
...
function splitTrainTest(points, mode) {
  ...
  if (mode === "logistic") {
    ...
    points.forEach((point, index) => {
      ...
      buckets[label].push({ point, index });
    });
    ...
    for (const bucket of buckets) {
      bucket.forEach(({ point, index }, position) => {
        if (position % 5 === 0) {
          test.push(point);
    ...
    if (train.length < minPointsForMode("logistic") || !hasBothClasses(train)) {
      return { train: points, test: [], testIndices: new Set() };
    }
...
function buildLogisticPreset(id = "logistic_circle") {
  ...
  for (let i = 0; i < 120; i += 1) {
    ...
    const x = radius * Math.cos(angle) + randomBetween(-0.18, 0.18);
    const y = radius * Math.sin(angle) + randomBetween(-0.18, 0.18);
...
const INITIAL_CHAT = [
  {
    role: "assistant",
    text:
      "مرحبًا بك في مختبر الانحدار. تعديل البيانات لا يبدأ التدريب تلقائيًا، اضغط «بدء» عندما تريد بدء التعلم."
  }
];

const ALGORITHM_LABELS = {
  batch: "دفعة كاملة",
  mini_batch: "دفعة مصغرة",
  momentum: "زخم"
};
...
function formatLossLabel(value) {
  return LOSS_LABELS[value] || value || "--";
}
...
export default function RegressionLabRenderer() {
  ...
  const [logisticFeatureDims, setLogisticFeatureDims] = useState(14);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && typeof window.requestIdleCallback === "function") {
      const idleId = window.requestIdleCallback(
        () => {
          if (!cancelled) setHeavyPanelsReady(true);
        },
        { timeout: 900 }
      );
      return () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const targetSteps = Math.max(1, Math.min(stepsPerFrame, performanceMode ? 90 : 140));
      const frameBudget = performanceMode ? Math.max(4.8, TRAIN_FRAME_BUDGET_MS - 1.4) : TRAIN_FRAME_BUDGET_MS;
      for (let i = 0; i < targetSteps; i += 1) {
        ...
        processedSteps += 1;
        if (processedSteps % 4 === 0 && performance.now() - frameStart >= frameBudget) {
          break;
        }
      }
      ...
      if (!hasLoss) {
        ...
        setLossHistory(model.history.slice(-420));
        setIsTraining(false);
        return;
      }

      epochRef.current += processedSteps;
      const now = performance.now();
      if (now - uiUpdateRef.current >= TRAIN_UI_UPDATE_MS) {
        uiUpdateRef.current = now;
        setEpoch(epochRef.current);
        const latestHistory = model.history.slice(-420);
        setLossHistory(latestHistory);
        if (testPoints.length > 0) {
          const latestTest = model.computeLoss(testPoints, lossFunction);
  ...
  const applyAssistantAction = (action) => {
    ...
    if (type === "set_logistic_degree") {
      ...
      setMode("logistic");
      setLogisticVariant("polynomial");
      setLogisticDegree(clamp(Math.round(value), 2, 8));
      return true;
    }
    ...
    if (type === "set_logistic_feature_dims") {
      ...
      setLogisticFeatureDims(clamp(Math.round(value), 4, 24));
      return true;
    }
  ...
  const sendMessage = async (overrideText) => {
    ...
    if (isOffline()) {
      setChatMessages((prev) => [
        ...prev,
        { role: "assistant", text: "الشات يحتاج اتصالاً بالإنترنت." }
      ]);
    ...
    try {
      ...
      const base =
        typeof data.explanation === "string" && data.explanation.trim()
          ...
          : applied
            ? "تم تطبيق الأمر داخل مختبر الانحدار."
            : "لم أتمكن من فهم الطلب بدقة.";
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          text: applied
            ? "تم التطبيق محليًا. للحصول على شرح أذكى، شغّل الخادم الخلفي على المنفذ 3002."
            : "الخادم غير متاح حاليًا. جرّب: غيّر الخوارزمية إلى زخم، غيّر الخسارة إلى BCE، واضبط معدل التعلم ثم ابدأ."
        }
      ]);
    } finally {
      setChatLoading(false);
  ...
  const trainingHint = useMemo(() => {
    if (mode === "logistic" && !hasBothClasses(trainingPoints)) {
      return "يلزم وجود الفئتين قبل التدريب. أضف نقاطًا للفئة 0 والفئة 1.";
    }
    if (!canTrain) return "أضف نقاطًا أكثر أولًا.";
    if (dataDirty) return "تم تعديل البيانات. اضغط «بدء» لتشغيل التدريب.";
    if (isTraining) return "التدريب قيد التنفيذ...";
    if (currentLoss === null) return "النموذج جاهز. اضغط «بدء».";
    if (isOverfitting) return "تحذير فرط مواءمة: خسارة الاختبار أعلى بكثير من خسارة التدريب.";
    if (currentLoss > getHighLossThreshold(mode, lossFunction)) {
      if (mode === "logistic" && logisticVariant === "linear") {
        return "الخسارة مرتفعة مع التحويل الخطي. جرّب التحويل متعدد الحدود أو الأسي في اللوجستي.";
      }
      if (mode === "logistic" && logisticVariant !== "linear") {
        return "الخسارة مرتفعة: زد أبعاد الميزات أو درجة كثير الحدود ثم أعد التدريب.";
      }
      return "الخسارة مرتفعة. جرّب خوارزمية أخرى أو دالة خسارة مختلفة.";
    }
    return "الملاءمة تبدو مستقرة. يمكنك تحسين البيانات ثم إعادة التدريب.";
  ...
  return (
    <div className={`reglab-shell ${isFullscreen ? "is-fullscreen" : ""}`}>
      {showIntro && (
        <LabIntroModal
          labId={INTRO_LAB_ID}
          slides={REGRESSION_INTRO_SLIDES}
          accentColor="#10b981"
          isArabic={isArabic}
      ...
      >
        <div>
          <h3>مختبر الانحدار</h3>
          <p>بيئة تفاعلية لملاءمة النماذج الخطية واللوجستية وتحليل سلوك التعلم.</p>
        </div>
            ...
            value={
              mode === "linear"
                ? linearVariant === "polynomial"
                  ? `خطي متعدد الحدود (d=${polynomialDegree})`
                  : "خطي"
                : logisticVariant === "polynomial"
                  ? `لوجستي متعدد الحدود (d=${logisticDegree}, f=${logisticFeatureDims})`
                  : logisticVariant === "exponential"
                    ? `لوجستي أسي (f=${logisticFeatureDims})`
                    : "لوجستي خطي"
            }
            color="#38bdf8"
          />
          <StatChip icon={<Binary size={13} />} label="الخوارزمية" value={formatAlgorithmLabel(algorithm)} color="#7dd3fc" />
          <StatChip icon={<Activity size={13} />} label="الخسارة" value={formatLossLabel(lossFunction)} color="#a5b4fc" />
          <StatChip icon={<Timer size={13} />} label="الحقبة" value={epoch} color="#3b82f6" />
          <StatChip
        ...
        <div className="reglab-header-actions">
          ...
          <button type="button" onClick={() => setPerformanceMode((value) => !value)}>
            {performanceMode ? "الأداء: مُفعّل" : "الأداء: مُعطّل"}
          </button>
          <button type="button" onClick={() => setShowRightStage((value) => !value)}>
            {showRightStage ? "إخفاء شاشة الرسم" : "إظهار شاشة الرسم"}
          </button>
      ...
      <div className="reglab-main">
        <aside className="reglab-left">
          ...
          >
            {heavyPanelsReady ? (
              ...
              <section className="reglab-panel reglab-heavy-placeholder">
                <strong>جارٍ تجهيز العرض ثلاثي الأبعاد...</strong>
                <p>يتم تحميل التصور عالي الكلفة أثناء وقت الخمول.</p>
              </section>
            )}
          </motion.div>

          {mode === "logistic" && heavyPanelsReady && (
            <motion.div
              initial={{ opacity: 0, x: -12 }}
              animate={{ opacity: 1, x: 0 }}
          ...
          {chatVisible && (
            ...
            >
              <div className="reglab-chat-header">
                <strong>شات الانحدار الذكي</strong>
                <span>جاهز</span>
              </div>
              <div className="reglab-chat-log" ref={chatScrollRef}>
                ...
                {chatLoading && <div className="reglab-msg assistant">يفكر...</div>}
              </div>
              ...
              <div className="reglab-chat-input">
                <input
                  ...
                  placeholder='جرّب: "اجعل الخوارزمية mini_batch" أو "set loss focal" أو "ابدأ"'
                />
        ...
        <section className={`reglab-center ${showRightStage ? "show-stage" : "hide-stage"}`}>
          {showRightStage ? (
            ...
            <div className="reglab-stage-hidden-note">شاشة الرسم مخفية الآن. استخدم زر الترويسة لإظهارها مجددًا.</div>
          )}
        </section>
