...
function drawLegend(ctx, width) {
  ...
  ctx.fillText("التدريب", width - 150, 16);
  ctx.fillText("الاختبار", width - 76, 16);
...
function drawChart(ctx, width, height, trainHistory, testHistory, mode, hoverState) {
  ...
  ctx.fillText("الخسارة", 10, 16);
  ctx.fillText("0", CHART_PAD.left - 12, height - CHART_PAD.bottom + 16);
  ctx.fillText(maxY.toFixed(3), 8, CHART_PAD.top + 4);
  ctx.fillText("الحقبة", width - 56, height - 10);
  drawLegend(ctx, width);
}

function formatEquation(mode, linearVariant, logisticVariant, model) {
  if (!model) return "";
  if (mode === "logistic") {
    if (logisticVariant === "polynomial") {
      return `p = sigmoid(φ_poly(x,y), degree=${model.degree || 3}, dims=${model.featureDims || 10})`;
    }
    if (logisticVariant === "exponential") {
      return `p = sigmoid(φ_exp(x,y), dims=${model.featureDims || 10})`;
    }
    const w1 = model.w1?.toFixed(3) ?? "0.000";
    const w2 = model.w2?.toFixed(3) ?? "0.000";
    const b = model.b?.toFixed(3) ?? "0.000";
    return `p = sigmoid(${w1}(x/5) + ${w2}(y/5) + ${b})`;
  }
...
function formatAlgorithmLabel(value) {
  if (value === "mini_batch") return "دفعة مصغرة";
  if (value === "momentum") return "زخم";
  if (value === "batch") return "دفعة كاملة";
  return value || "--";
}

function formatLossLabel(value) {
  if (value === "bce") return "BCE";
  if (value === "focal") return "فوكال";
  if (value === "mae") return "MAE";
  if (value === "mse") return "MSE";
  if (value === "hinge") return "هنج";
  if (value === "log_cosh") return "لوغ-كوش";
  if (value === "huber") return "Huber";
  return value || "--";
}
    ...
    <section className="reglab-panel reglab-visualizer">
      ...
      <div className="reglab-loss-wrap" ref={canvasWrapRef}>
        ...
        {hoverTooltip && (
          ...
          >
            <p>الحقبة: {hoverState.epoch}</p>
            <p>التدريب: {Number.isFinite(hoverState.trainValue) ? hoverState.trainValue.toFixed(5) : "--"}</p>
            <p>الاختبار: {Number.isFinite(hoverState.testValue) ? hoverState.testValue.toFixed(5) : "--"}</p>
            {Number.isFinite(hoverState.trainValue) && Number.isFinite(hoverState.testValue) && (
              <p>الفجوة: {(hoverState.testValue - hoverState.trainValue).toFixed(5)}</p>
            )}
          </div>
        )}
      </div>
      <div className="reglab-equation">
        ...
        {canManualTune ? (
          ...
          <code>{formatEquation(mode, linearVariant, logisticVariant, model)}</code>
        )}
        <div className="reglab-equation-meta">
          <span>{formatAlgorithmLabel(algorithm)}</span>
          <span>{formatLossLabel(lossFunction)}</span>
        </div>
      </div>
      <div className="reglab-visualizer-stats">
        ...
        <p>
          النقاط: <span className="reglab-mono">{pointsCount}</span> ({trainCount}/{testCount})
        </p>
        <p>
          التدريب: <span className="reglab-mono">{currentLoss !== null ? currentLoss.toFixed(5) : "--"}</span>
        </p>
        <p>
          الاختبار: <span className="reglab-mono">{currentTestLoss !== null ? currentTestLoss.toFixed(5) : "--"}</span>
        </p>
        {isOverfitting && <p className="reglab-overfit-note">تم رصد فرط مواءمة: خسارة الاختبار أعلى بكثير من خسارة التدريب.</p>}
      </div>
    </section>
  );
}
