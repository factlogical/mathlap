  ...
  useEffect(() => {
    ...
    if (drawingEnabled) {
      const hint = isDrawing
        ? `جارٍ الرسم... ${drawnPoints.length} نقطة (حرّر لإغلاق المنحنى).`
        : drawMessage || "وضع الرسم الحر: اسحب لرسم منحنى مغلق.";
      ctx.fillText(hint, 16, 20);
    } else {
      ctx.fillText("انقر مركز مستطيل لمطابقة نقطة التصادم المقابلة في 3D.", 16, 20);
    }
  }, [
    bounds,
    curvePoints,
