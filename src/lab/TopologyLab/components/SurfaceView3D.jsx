  ...
  const traces = useMemo(() => {
    ...
    if (Array.isArray(surface?.grid) && surface.grid.length > 0) {
      data.push({
        ...
        hovertemplate: "المنتصف=(%{x:.2f}, %{y:.2f})<br>المسافة=%{z:.2f}<extra></extra>",
        name: "السطح الطوبولوجي"
      });
    }
    ...
    if (showIntersections && Array.isArray(collisions) && collisions.length > 0) {
      data.push({
        ...
        hovertemplate: "تصادم %{customdata}<br>(%{x:.2f}, %{y:.2f}, %{z:.2f})<extra></extra>"
      });
    }
    ...
    if (activeHighlight) {
      data.push({
        ...
        hovertemplate: "التصادم المحدد<br>(%{x:.2f}, %{y:.2f}, %{z:.2f})<extra></extra>"
      });
    }
    ...
    <div className="topology-surface-wrap">
      ...
      {(!surface?.grid || surface.grid.length === 0) && (
        <div className="topology-surface-empty">
          أضف أو ارسم منحنى مغلقًا لبناء السطح الطوبولوجي ثلاثي الأبعاد.
        </div>
      )}
    </div>
  );
