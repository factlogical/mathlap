...
const STEPS = [
  ...
  },
  {
    ...
    content:
      "النقاط المعلّمة في 2D تمثل أزواجًا مرشحة. إذا تساوى منتصف القطعة وطولها لزوجين نحصل على مستطيل.",
    focus: "curve",
    pointer: { x: 66, y: 52, label: "زوجان مرشحان" },
    curveHint: "راقب الأزواج التي تشترك في midpoint وطول القطعة.",
    diagram: "rectangle"
  },
  {
    ...
    surfaceHint: "كل نقطة على السطح تمثل زوج نقاط من 2D.",
    diagram: "lift"
  },
  {
    ...
    focus: "surface",
    pointer: { x: 50, y: 36, label: "تصادم" },
    surfaceHint: "النقطة البرتقالية هنا تعني حلًا مستطيليًا على المنحنى.",
    diagram: "collision"
  },
  {
    title: "تجربة عملية",
    content:
      "الآن قارن بين المنحنيات (مثل figure8 وtrefoil) مع تفعيل «إظهار كل المستطيلات في 2D».",
    focus: "curve",
    pointer: { x: 72, y: 48, label: "قارن الكثافة" },
    preset: "figure8",
    curveHint: "فعّل عرض كل المستطيلات للمقارنة البصرية المباشرة.",
    diagram: "compare"
  }
];
  ...
  if (type === "rectangle") {
    return (
      <svg viewBox="0 0 210 70" className="topology-tutorial-svg" aria-hidden="true">
        ...
        <text x="110" y="38">نفس المنتصف</text>
      </svg>
    );
  }
  if (type === "compare") {
    return (
      <svg viewBox="0 0 210 70" className="topology-tutorial-svg" aria-hidden="true">
        ...
        <text x="12" y="14">مقارنة figure8 و trefoil</text>
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 210 70" className="topology-tutorial-svg" aria-hidden="true">
      ...
      <text x="130" y="16">شرط الزوج</text>
    </svg>
  );
}
...
export default function TutorialMode({ onClose, onStepChange }) {
  ...
  useEffect(() => {
    ...
    const suppress = () => setPointerSuppressed(true);
    target.addEventListener("pointerdown", suppress);
    target.addEventListener("wheel", suppress, { passive: true });
    target.addEventListener("touchstart", suppress, { passive: true });

    return () => {
      target.removeEventListener("pointerdown", suppress);
      target.removeEventListener("wheel", suppress);
      target.removeEventListener("touchstart", suppress);
    };
  }, [current.focus]);
    ...
    <div className="topology-tutorial-overlay" aria-live="polite">
      ...
      <div className="topology-tutorial-dock">
        <div className="topology-tutorial-header">
          ...
          <button type="button" onClick={onClose} aria-label="إغلاق العرض التعريفي">
            ×
          </button>
        </div>
        ...
        <p>{current.content}</p>
        {pointerSuppressed && (
          <div className="topology-tutorial-note">
            تم تحريك الرسم. انتقل للخطوة التالية لإعادة التأشير بدقة.
          </div>
        )}
