...
function resolveActivationKey(unit, fallback = "relu") {
  const candidate = String(unit?.activation || fallback || "relu").toLowerCase();
  return Object.prototype.hasOwnProperty.call(ACTIVATIONS, candidate) ? candidate : "relu";
}

export function computeUnit(x, unit, fallbackActivation = "relu") {
  const w = Number(unit?.w) || 0;
  const b = Number(unit?.b) || 0;
  const z = w * x + b;
  const activationKey = resolveActivationKey(unit, fallbackActivation);
  const activation = getActivation(activationKey);
  const a = activation.fn(z);
  return { z, a, active: a > 0, activationKey };
}

export function computeUnits(x, units, activationKey = "relu") {
  return units.map((unit) => computeUnit(x, unit, activationKey));
}
