...
function toCanvas(point, bounds, width, height, pad = 24) {
...
function fromCanvas(x, y, bounds, width, height, pad = 24) {
  const usableW = Math.max(1, width - pad * 2);
  const usableH = Math.max(1, height - pad * 2);
...
export default function CurveCanvas2D({
  ...
  const bounds = useMemo(() => curveBounds(curvePoints, 0.65), [curvePoints]);
  ...
  useEffect(() => {
    ...
    rectangles.forEach((rect, index) => {
      ...
      ctx.strokeStyle = `rgba(${stroke === "#f59e0b" ? "245, 158, 11" : "52, 211, 153"}, ${alpha})`;
