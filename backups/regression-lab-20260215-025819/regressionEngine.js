...
function createLogisticFeatures(x, y, variant = "linear", degree = 2, featureDims = 10) {
  ...
  if (variant === "polynomial") {
    const d = Math.max(2, Math.min(5, Math.round(degree)));
    const features = [1, sx, sy];
...
export class LogisticRegression {
  constructor(options = {}) {
    ...
    this.variant = ["linear", "polynomial", "exponential"].includes(options.variant)
      ? options.variant
      : "linear";
    this.degree = Math.max(2, Math.min(5, Math.round(options.degree || 3)));
  ...
  step(points, options = {}) {
    ...
    for (const point of active) {
      ...
      const dZ = logisticLossDerivative(p, y, loss, z);
