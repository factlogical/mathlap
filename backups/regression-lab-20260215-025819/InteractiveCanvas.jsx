...
function drawNonlinearBoundary(ctx, model, transform, xRange, yRange) {
  const [xMin, xMax] = xRange;
  const [yMin, yMax] = yRange;
  const { toCanvas } = transform;
  const samplesX = 90;
  ...
  for (let i = 0; i <= samplesX; i += 1) {
    const x = xMin + ((xMax - xMin) * i) / samplesX;
    let bestY = yMin;
    let bestGap = Infinity;
    for (let j = 0; j <= samplesY; j += 1) {
      const y = yMin + ((yMax - yMin) * j) / samplesY;
      const gap = Math.abs(model.predict(x, y) - 0.5);
      if (gap < bestGap) {
        bestGap = gap;
        bestY = y;
      }
...
function drawLogistic(ctx, width, height, points, model, transform, xRange, yRange, testPointIndices, tool, selectedClass) {
  ...
  if (
    ...
    drawNonlinearBoundary(ctx, model, transform, xRange, yRange);
  }
  ...
  ctx.font = "12px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillText(`Tool: ${tool} | Class: ${selectedClass} | map: ${model?.variant || "linear"}`, 14, 22);
}
