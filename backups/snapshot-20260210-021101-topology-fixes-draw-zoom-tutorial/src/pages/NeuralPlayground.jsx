...
const NeuralPlayground = () => {
  ...
  }, [grid.x, grid.y, dataset, trainingStep, mode, boundaryTick]);
  ...
  useEffect(() => {
    ...
    const trainStep = (timestamp) => {
      ...
      if (seen > 0) {
        ...
        const shouldUpdateUI = uiCounter % 2 === 0;
        uiCounter += 1;
        if (shouldUpdateUI) {
        ...
        <aside className="neural-panel neural-problem">
          ...
          <div className="neural-card neural-card--training">
            ...
            <div className="neural-training">
              ...
              <div className={`neural-lr ${copilotState.type === 'alert' ? 'highlight' : ''}`}>
                <div>
  ...
  const body = !hasNetwork ? (
    ...
    <div className="neural-math-grid">
      <div className="neural-math-card">
        ...
        <BlockMath math="y = A \oplus B" throwOnError={false} />
    ...
    <>
      <div className="neural-math-row">
        ...
        <InlineMath math="\times" throwOnError={false} />
        <div className="neural-math-block" ref={matrixRef} onMouseMove={handleMatrixMove} onMouseLeave={handleMatrixLeave}>
          <p>Weights</p>
      ...
      <div className="neural-math-grid">
        <div className="neural-math-card">
          <h4>Gradients</h4>
          <BlockMath math={`\frac{\partial L}{\partial W^{[1]}} = ${dWLatex}`} throwOnError={false} />
          <BlockMath math={`\frac{\partial L}{\partial b^{[1]}} = ${dBLatex}`} throwOnError={false} />
        ...
        <div className="neural-math-card neural-math-mini">
          ...
          />
          <p className="neural-math-caption">Hover a weight to highlight the link.</p>
        </div>
      </div>
    </>
  );
    ...
    <div className="neural-math-view" ref={viewRef}>
      ...
      {hoverInfo && !isSymbolic && (
        <div className="neural-math-tooltip" style={{ left: hoverInfo.x, top: hoverInfo.y }}>
          Weight from Neuron {hoverInfo.col + 1} to Neuron {hoverInfo.row + 1}
...
const mapSpeedToDelay = (value) => {
  ...
  return Math.round(500 - ratio * 500);
};
...
const truncateRows = (rows, maxRows = 6) => {
  ...
  if (rows.length <= maxRows) {
    return { rows, indices: rows.map((_, idx) => idx) };
  }
  return {
