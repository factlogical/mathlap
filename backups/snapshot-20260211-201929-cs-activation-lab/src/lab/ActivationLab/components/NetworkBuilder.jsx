...
function sampleCurves(units, targetFn, xMin, xMax, highlightUnit) {
  ...
  const allValues = [...targetVals, ...outputVals];
  if (highlightUnit !== null && Number.isInteger(highlightUnit)) {
    allValues.push(...contributions[highlightUnit]);
  } else {
    contributions.forEach((arr) => allValues.push(...arr));
  }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const toC = (x, y) => ({
        ...
        cy: PAD.top + ph - ((y - sampled.yMin) / (sampled.yMax - sampled.yMin)) * ph
      });
      ...
      units.forEach((unit, i) => {
        ...
        sampled.xVals.forEach((x, idx) => {
          const { cx, cy } = toC(x, sampled.contributions[i][idx]);
      ...
      ctx.beginPath();
      sampled.xVals.forEach((x, idx) => {
        const { cx, cy } = toC(x, sampled.targetVals[idx]);
        if (idx === 0) ctx.moveTo(cx, cy);
        else ctx.lineTo(cx, cy);
      });
      ctx.stroke();
      ...
      ctx.beginPath();
      sampled.xVals.forEach((x, idx) => {
        const { cx, cy } = toC(x, sampled.outputVals[idx]);
        if (idx === 0) ctx.moveTo(cx, cy);
        else ctx.lineTo(cx, cy);
      });
      ctx.stroke();
