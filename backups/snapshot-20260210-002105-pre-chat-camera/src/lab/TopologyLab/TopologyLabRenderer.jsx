...
function buildLocalReply(text, context) {
  ...
  if (/trefoil|عقد/.test(lower + ar)) {
    return withAction(
      ...
      "اسحب بالماوس داخل نافذة 3D لتغيير الزاوية وكشف نقاط التصادم المخفية.",
      {
        type: "change_curve",
  ...
  if (/اوقف|وقف|stop|freeze/.test(lower + ar) && /دور|spin|rotate/.test(lower + ar)) {
    return withAction(
      "تم. العرض ثلاثي الأبعاد الآن ثابت بدون حركة تلقائية.",
      "يمكنك تدوير المشهد يدويًا بالماوس عند الحاجة."
    );
  }

  if (/دور|rotate|spin/.test(lower + ar)) {
    return withAction(
      "في هذا الإصدار أوقفت الحركة التلقائية للمشهد 3D حتى تبقى القراءة مستقرة.",
      "دوّر المشهد يدويًا بالسحب داخل نافذة 3D."
    );
  }
  ...
  return withAction(
    ...
    "جرّب: دائرة، قطع ناقص، أو شرح معنى midpoint والمسافة."
  );
}
