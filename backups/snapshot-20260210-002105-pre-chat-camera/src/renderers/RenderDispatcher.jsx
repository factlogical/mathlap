 ...
 */
const RenderDispatcher = ({ spec, action, onAnalysis }) => {
    ...
    if (!data.math) {
        ...
        if (data.mode === 'equation' || data.mode === 'recurrence' || data.mode === 'data') {
            return <ScalarPlotRenderer spec={spec} action={action} />;
        }
    ...
    switch (kind) {
        ...
        case "data_set":
            ...
            if (data.math.data?.vectors) {
                return <VectorPlotRenderer spec={spec} onAnalysis={onAnalysis} />;
            }
            return <ScalarPlotRenderer spec={spec} action={action} />;
