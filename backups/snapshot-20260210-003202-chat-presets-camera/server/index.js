...
if (!OPENAI_KEY) {
  console.error("â Critical Error: OPENAI_API_KEY not found in .env");
  process.exit(1);
}
...
function asksFor3DMode(text) {
  ...
  return (
    ...
    /(Ø«ÙØ§Ø«Ù|Ø³Ø·Ø­|ÙØ³ØªÙÙ|Ø¹ÙÙØ¯Ù|ÙØ´ØªÙØ§Øª Ø¬Ø²Ø¦ÙÙ)/.test(normalizedArabic)
  );
}
...
function asksFor2DMode(text) {
  ...
  return (
    ...
    /(Ø«ÙØ§Ø¦Ù|Ø§ÙÙØ§Ø·Ø¹|Ø§ÙÙÙØ§Ø³)/.test(normalizedArabic)
  );
}
...
function asksSecantToTangent(text) {
  ...
  const arabicTransition =
    ...
    /ÙØ±ÙÙ|ÙØ±ÙÙÙ|Ø§Ø±ÙÙ|Ø§Ø±ÙÙÙ|ÙÙÙ|ÙØµÙØ±|ÙØªØ­ÙÙ/.test(normalizedArabic);
...
function hasVisualIntent(text = "") {
  return /(Ø§Ø±Ø³Ù|ØºÙÙØ±|ØºÙØ±|Ø¨Ø¯ÙÙ|Ø¨Ø¯Ù|Ø¬Ø±ÙØ¨|Ø¬Ø±Ø¨|Ø§Ø¹Ø±Ø¶|Ø®ÙÙ|Ø®Ù|Ø¶Ø¹|Ø­Ø±Ù|Ø­Ø±ÙÙ|Ø³ÙÙ|Ø³ÙÙÙ|plot|draw|show|set)/i.test(String(text));
}

function isJustAskingExample(text = "") {
  return /(Ø§Ø¹Ø·ÙÙ ÙØ«Ø§Ù|ÙØ«Ø§Ù ÙÙØ·|Ø§Ø´Ø±Ø­|ÙØ¶Ø­|ÙØ³Ø±|ÙØ§ ÙØ¹ÙÙ|ÙØ§ ÙÙ)/i.test(String(text)) && !hasVisualIntent(text);
}
...
Mission:
1) Explain topology ideas in simple Arabic/English.
...
Return JSON only with this schema:
{
  ...
  "action": {
    "type": "change_curve|set_resolution|toggle_collisions|highlight_rectangle",
    "params": {}
  } | null
}
...
app.post("/api/interpret", async (req, res) => {
  try {
    ...
    console.log(`ð¡ Sending request to OpenAI (${MODEL_NAME}) [Mode: ${mode}]...`);

    const completion = await openai.chat.completions.create({
      model: MODEL_NAME,
      messages: buildMessages(promptText, mode, context),
      temperature: mode === "derivative_chat" || mode === "topology_chat" ? 0.3 : 0.1,
      response_format: { type: "json_object" } // Enforce JSON mode
    });

    const text = completion.choices[0].message.content;
    console.log("ð¥ OpenAI Response:", text);
    ...
    try {
      ...
      console.error("â JSON Parse Error:", text);
    ...
    if (mode === "topology_chat") {
      ...
      if (action?.type === "toggle_collisions" || action?.type === "toggle_intersections") {
        action = {
          ...
          params: { show: Boolean(action.params?.show) }
        };
      }
    ...
    if (mode === 'lab_animation' || mode === 'lab_chat') {
      ...
      if (parsedData.error) {
        console.warn("â ï¸ Schema Validation Warning:", parsedData.error);
      }
    }
...
app.listen(PORT, () => {
  ...
  console.log(`ð¤ AI Model: ${MODEL_NAME}`);
});
