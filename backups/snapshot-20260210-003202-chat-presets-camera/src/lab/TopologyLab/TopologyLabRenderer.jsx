...
function fallbackLocalReply(text, context) {
  ...
  const presets = [
    { key: "circle", re: /(circle|دائره|دائرة)/ },
    { key: "figure8", re: /(figure\s*8|رقم\s*8|ثمانية)/ },
    { key: "lemniscate", re: /(lemniscate|لانهاية|لا نهائي|infinity)/ },
    { key: "trefoil", re: /(trefoil|عقده|عقدة)/ },
    { key: "spiral", re: /(spiral|حلزون|حلزوني)/ },
  ...
  const resolutionMatch = raw.match(/(?:resolution|دقة|الدقة)\s*(?:to|=|الى|إلى)?\s*(\d{2,3})/i);
  if (resolutionMatch) {
