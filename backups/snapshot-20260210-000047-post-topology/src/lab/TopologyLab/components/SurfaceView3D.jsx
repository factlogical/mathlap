...
export default function SurfaceView3D({
  ...
  const [orbitAngle, setOrbitAngle] = useState(0.3);

  useEffect(() => {
    if (!animateSurface) return undefined;
    const timer = window.setInterval(() => {
      setOrbitAngle((prev) => prev + 0.018);
    }, 60);
  ...
  const layout = useMemo(() => {
    const eye = {
      x: 2.05 * Math.cos(orbitAngle),
      y: 2.05 * Math.sin(orbitAngle),
      z: 1.15
    };
